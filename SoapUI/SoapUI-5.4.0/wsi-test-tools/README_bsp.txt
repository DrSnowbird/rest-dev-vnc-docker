README_bsp10.txt
  ...
  Copyright (c) 2007 by The Web Services-Interoperability Organization (WS-I) and 
...
BSP Java validation tools


Prereqs:
A JDK (5.0 or greater)

QUICKSTART:
---

To use the monitor tool, run:
monitor.bat -config <configFile>

monitor-config-sample.xml is a sample monitor configuration file which you can
use right out of the box.  The following command will set up a monitor that
listens on port 7070 and forwards the requests to port 8080:
monitor.bat -config monitor-config-sample.xml

---

To use the analyzer tool, run:
analyzerV<Version>.bat -config <configFile>

  <Version> - 11 will analyze according to BSP 1.1 Profile
            - 10 will analyze according to BPS 1.0 Profile

analyzer-config-sample.xml is a sample analyzer configuration file which you can
use right out of the box to analyze the provided sample log file,
log-sample.xml.  The following command will generate a report-sample.xml file:
analyzerV11.bat -config analyzer-config-sample.xml

You can view the generated reports using a web browser.

---

You can find further documentation in the UserGuide.pdf file.  The analyzer
documentation has not yet been updated for the Basic Security Profile, but
the user guide should be still mostly applicable.
