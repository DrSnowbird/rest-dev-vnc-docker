...
<!--
  Formatting for WS-I Profile Test Assertion Document used by the Testing Tools
	...
	Copyright (c) 2002-2006 by The Web Services-Interoperability Organization (WS-I) and 
                        ...
                        <dl>
                            ...
                            <xsl:for-each select="wsi-assertions:editors/wsi-assertions:person">
                                <dd>
                                    ...
                                    (<a href="mailto:{@href}"><xsl:value-of select="@href"/></a>)     
                                </dd>
                            </xsl:for-each>
...
<a href="mailto:wsi-tools@ws-i.org">wsi-tools@ws-i.org</a>. 
  ...
  <p/>
  <p class="toc">
  <a href="#documentConventions">Document Conventions</a><br/>
  <a href="#profileDefinitions">Profile Definitions</a><br/>
  ...
  <hr style="color : black;"/>
  <h2><a name="documentConventions">Document Conventions</a></h2>
  <p><xsl:apply-templates select="wsi-assertions:conventions" /></p>
  <p><xsl:call-template name="namespaces"/></p>
  
	<p>A "candidate" element is one that is to be verified for conformance.  
The <a href="http://www.ws-i.org/Testing/Specs/AnalyzerFunctionalSpecification_1.02.pdf">analyzer 
...
<xsl:template match="wsi-assertions:testAssertion" >
  ...
  <xsl:choose>
  <xsl:when test="@enabled='false'">
    <xsl:variable name="assertionID" select="@id"/>
    <h3 style="background-color : {$highlightColor4};" id="{$assertionID}">Test Assertion: <xsl:value-of select="@id"/>
    
    </h3>    
  </xsl:when>
  <xsl:otherwise>
    <xsl:variable name="assertionID" select="@id"/>
    <h3 id="{$assertionID}">Test Assertion: <xsl:value-of select="@id"/>
    <!-- REMOVE:
   	<xsl:if test="//wsi-assertions:testAssertion/wsi-assertions:additionalInfo">
...
<xsl:template name="namespaces">
            <p>This document uses a number of namespace prefixes throughout;
            ...
            <ul>
            <xsl:for-each select="//wsi-assertions:namespaces/wsi-assertions:namespace">
                <li><strong><xsl:value-of select="@prefix"/></strong> - "<xsl:value-of select="."/>"</li>
            </xsl:for-each>
            </ul>
</xsl:template>
...
<xsl:template match="wsi-assertions:p">
	<p><xsl:value-of select="."/></p>
</xsl:template>
<xsl:template match="wsi-assertions:ul">
	<ul><xsl:apply-templates select="./*"/></ul>
</xsl:template>
<xsl:template match="wsi-assertions:li">
